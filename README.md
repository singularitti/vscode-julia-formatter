...
- `juliaFormatter.overwriteFlags`: Whether to overwrite the settings of the
  specified style with the `JuliaFormatter` flags specified in vs-code's
  settings. `false` by default.
...
- `juliaFormatter.alignAssignment`: If `true`, align the `=`-like operators.
